    ...
    export default {
        ...
        methods: {
            async getLunbotu() {
                const {
                    data
                } = await this.axios.get("getthumimages/" + this.id);
                if (data.status === 0) {
                    // 先循环轮播图数组的每一项，为 item 添加 img 属性，因为 轮播图 组件中，
                    //只认识 item.img， 不认识 item.src
            ...
            async getGoodsInfo() {
                ...
                const {
                    data
                } = await this.axios.get("goods/getinfo/" + this.id);
                if (data.status === 0) {
